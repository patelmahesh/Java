...
public class BinaryMap {
    ...
    public static void main(String[] args) {
       ...
       int maxGap = 0;
        String s = Integer.toBinaryString(N).replaceAll("1", "1,1");
        String[] arr = s.split(",");
        for (int i = 0; i < arr.length; i++) {
            if (arr[i].contains("0") && (arr[i].charAt(0) == '1') && (arr[i].charAt(arr[i].length() - 1) == '1')) {
                int length = arr[i].replaceAll("1", "").trim().length();
                if (length > maxGap) {
                    maxGap = length;
                }
            }
